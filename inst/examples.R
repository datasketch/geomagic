...
g <- gg_choropleth_GcdNum(data = NULL,
...
#ggsave(g, filename = "latam.png", bg = "transparent")
# Examples GcdNum ---------------------------------------------------------
...
gg_choropleth_GcdNum(data = data,
                     opts = list(
                       ...
                       marks = c('','.'),
                       projection_name = "ortho",
                       projection_orientation = c(41, -74, 0)))
...
gg_choropleth_Gnm(data = NULL,
                  opts = list(
                    titles = 'Mapa sin datos')
                  )
gg_choropleth_Gnm(data = data)

data2 <- data.frame(id = "Arauca", num = 100)
gg_choropleth_GnmNum(data = data2, mapName = "col_departments")

gg_choropleth_Gnm(data = data,
                  mapName = 'africa_countries',
                  opts = list(
                    title = "Mapa con datos",
                    title_color = "#FFFCCC",
                    subtitle = "porq es una prueba",
                    subtitle_color = "#FFFCCC",
                    caption =  'datasketch',
                    caption_color = "#FFFCCC",
                    border_width = 0.5,
                    border_color = '#FFAA12',
                    colors = c('#FCCADF', '#AA12FC'),
                    naColor = '#FDDFCC',
                    opacity = 0.9,
                    scale = 'discrete',
                    background = '#000000',
                    legend_borderColor = 'green',
...
gg_bubbles_GlnGltCat(data = dataC,
                     opts = list(
                       legend_show = c(TRUE,FALSE),
                       legend_title = c("lege1", "")
                       )
                     )
dataC <- sampleData('Gln-Glt-Cat', 1000)
gg_bubbles_GlnGltCat(data = dataC,
                     opts = list(
                       legend_title = c("legend1", "legend2")
                       )
                     )
